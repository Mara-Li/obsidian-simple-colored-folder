...
import { SimpleColoredFolderModal } from "./modals";
import { SimpleColoredFolderSettings, DEFAULT_SETTINGS } from "./interfaces";
...
export default class SimpleColoredFolder extends Plugin {
	...
	async onload() {
		...
		this.addCommand({
			...
			callback: () => {
				new SimpleColoredFolderModal(this.app).open();
			}
		});
